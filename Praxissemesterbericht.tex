...
Polarion hinterlegt die Eintrage im HTML Format, daher wird die Dokumenation als HTML hochgeladen. Wie in Kapitel \ref{polarion} näher erläutert, Polarion ist eine Anforderungs-Management-Software. Der fertige HTML-Code wird aus einem Jinja-template generiert. Jinja ist ein template Engine, es hat spezielle Platzhalter-Syntax um an diese Stellen verschiedene Daten einfügen zu können.
...
Abbildung \ref{fig:jinja_python_code} zeigt den Python Code der die Daten in die template-file gibt und rendert. Zeile 6 wird das Enviroment mit dem aktuellen Verzeichnis geladen, in Zeile 8 wird daraus dann die template file geladen, Zeile 11 bis 15 sind ein dictionary wo die Keys den Variablen im Template entsprechen. In Zeile 18 werden die Daten in das template übergeben und gerenderd, anschließend wird die gerenderte file erst in der Konsole ausgegeben und dann in einer separaten html-file gespeichert.
Abbildung \ref{fig:jinja_fertige_html_file} zeigt dann diese fertige file. \cite{jinja_doku} %TODO: hier mal ChatGPT nach seiner Meinung fragen - Grammatik/Rechtschreibung
